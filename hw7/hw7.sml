...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Intersect(e1,e2) => intersect(eval_prog(e1,env), eval_prog(e2, env))
			| Shift (deltaX, deltaY, gexp) => 
        case eval_prog(gexp, env) of
            NoPoints => NoPoints
						| Point (x1,y1) => Point (x1 + deltaX, y1 + deltaY) 
						| Line (m, b) => Line (m, b + deltaY - m * deltaX)
						| VerticalLine x => VerticalLine (x + deltaX)
						| LineSegment (x1,y1,x2,y2) =>
										LineSegment (x1 + deltaX, y1 + deltaY,
																x2 + deltaX, y2 + deltaY)
																
										| _ => raise Impossible("Cannot shift this type")
...
fun preprocess_prog(e) = 
    case e of
      LineSegment (x1,y1,x2,y2) => if real_close (x1,x2) then
