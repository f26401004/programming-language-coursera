...
class Point < GeometryValue
  ...
  def intersectVerticalLine verticalLine
    if real_close(y, verticalLine.x)
      self
    else
      NoPoints.new
    end
  ...
  private
  def inbetween(value, startValue, endValue)
    (startValue - GeometryExpression::Epsilon <= value && value <= endValue + GeometryExpression::Epsilon) || (endValue - GeometryExpression::Epsilon <= value && value <= startValue + GeometryExpression::Epsilon)
  end
end
...
class Line < GeometryValue
  ...
  def intersectVerticalLine verticalLine
    Point.new(verticalLine.x, m * verticalLine.x + b)
  end
  def intersectWithSegmentAsLineResult segment
    segment
  end
...
class VerticalLine < GeometryValue
  ...
  def intersectVerticalLine verticalLine
    if real_close(x, verticalLine.x)
      self
    else
      NoPoints.new
    end
...
class LineSegment < GeometryValue
  ...
  def intersectVerticalLine verticalLine
    verticalLine.intersectLineSegment self
  end
  def intersectWithSegmentAsLineResult segment
    if real_close(x1, x2)
      lox1=x1; loy1=y1; lox2=x2; loy2=y2; hix1=segment.x1; hiy1=segment.y1; hix2=segment.x2; hiy2=segment.y2
  		if y1 > segment.y1
  			lox1=segment.x1; loy1=segment.y1; lox2=segment.x2; loy2=segment.y2; hix1=x1; hiy1=y1; hix2=x2; hiy2=y2
  		end
